...
| RQ-S1 | Remote TLS connect gate (`SPECTRA_ALLOW_INSECURE_REMOTE`) | Correctness-only waiver: unit tests in `remote_security`; connect-path microbench not measured (fail-closed scheme check is O(1) string prefix). |
